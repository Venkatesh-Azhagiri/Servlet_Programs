(function($) {
	...
	$.fn.RSV.validate = function(event) {
		...
		for ( var i = 0; i < rules.length; i++) {
			...
			if (requirement != "function" && options.errorFieldClass) {
				if (form[fieldName].type == undefined) {
					for ( var j = 0; j < form[fieldName].length; j++) {
						if ($(form[fieldName][j]).hasClass(
								options.errorFieldClass))
							$(form[fieldName][j]).removeClass(
									options.errorFieldClass);
					}
				} else {
			...
			case "valid_email":
				if (form[fieldName].value
						&& !isValidEmail(form[fieldName].value)) {
					if (!processError(form[fieldName], errorMessage))
						return false;
				}
				break;
			case "valid_date":
				...
				if (date_flag == "later_date")
					...
					isLaterDate = false;
				if (!isValidDate(form[fieldName].value, form[fieldName2].value,
	...
	function displayHTMLErrors(f, errorInfo) {
		...
		for ( var i = 0; i < errorInfo.length; i++) {
			errorHTML += options.errorHTMLItemBullet + errorInfo[i][1]
					+ "<br />";
